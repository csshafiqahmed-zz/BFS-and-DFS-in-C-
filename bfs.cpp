...
int main() {
    ...
    clock_t t1;
    t1 = clock();
    cout<<"Entern Goal : "; int goal;
    cin>>goal;
 
    /** Explores all vertices findable from vertex 1 */
    int done=g.BFS(1, goal);
    float diff = (double)(clock() - t1)/CLOCKS_PER_SEC ;
    if(done==0){
    	cout<<"Goal not found.";
    }else if(done==goal){
    	cout<<"Goal found."<<endl;
    	cout <<endl<< "The time taken for Breadth first search: "<< diff <<" milli seconds"<< endl;
    }else{
